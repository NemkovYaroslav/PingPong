...
InputDevice::InputDevice() {
	...
	Rid[0].hwndTarget = Game::GetInstance()->GetDisplay().get()->GetHWnd();

	Rid[1].usUsagePage = 0x01;
	Rid[1].usUsage = 0x06;
	Rid[1].dwFlags = 0;   // adds HID keyboard and also ignores legacy keyboard messages
	Rid[1].hwndTarget = Game::GetInstance()->GetDisplay().get()->GetHWnd();
...
void InputDevice::OnMouseMove(RawMouseEventArgs args)
{
	...
	POINT p;
	GetCursorPos(&p);
	ScreenToClient(Game::GetInstance()->GetDisplay().get()->GetHWnd(), &p);
